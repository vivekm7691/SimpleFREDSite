...
class GeminiService:
    ...
    def __init__(
        self, api_key: Optional[str] = None, model_name: str = "models/gemini-2.5-flash"
    ):
        """
        ...
        if not self.api_key.startswith("AIza"):
            raise ValueError(
                f"Invalid API key format. Gemini API keys should start with 'AIza'. Got: {self.api_key[:10]}..."
            )

        # Logging (masked for security - only first 10 and last 5 characters)
        logger.info("=== GeminiService initializing ===")
        logger.info(
            f"API Key: {self.api_key[:10]}...{self.api_key[-5:]} (length: {len(self.api_key)})"
        )
        ...
        logger.info(
            f"genai.configure() called with key: {self.api_key[:10]}...{self.api_key[-5:]}"
        )
...
"""
    ...
    async def _generate_content_async(self, prompt: str) -> str:
        ...
        try:
            logger.info(
                f"Calling model.generate_content() with model: {self.model_name}"
            )
            ...
            logger.error(
                f"Exception in _generate_content_async: {error_msg}", exc_info=True
            )
            if (
...
def get_gemini_service() -> GeminiService:
    ...
    if _gemini_service is None:
        ...
        logger.info(
            f"Using existing GeminiService instance with model: {_gemini_service.model_name}"
        )
