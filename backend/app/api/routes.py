...
async def summarize_data(request: SummarizeRequest):
    ...
    logger.info(f"Request data keys: {list(request.data.keys()) if isinstance(request.data, dict) else 'N/A'}")
    ...
    try:
        ...
        logger.info(f"Using API Key: {api_key[:10]}...{api_key[-5:]} (length: {len(api_key)})")
        ...
        logger.error(f"Exception in summarize_data: {type(e).__name__}: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
